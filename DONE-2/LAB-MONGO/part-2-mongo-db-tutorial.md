# MongoDB Tutorial - Практическое занятие

## 🎯 Цель занятия
Изучить основы работы с MongoDB через командную строку и Python

## 📚 Теория

### Что такое MongoDB?
- **NoSQL** документная база данных
- Хранит данные в формате **BSON** (Binary JSON)
- **Коллекции** вместо таблиц
- **Документы** вместо строк

### Структура данных
```
Database (База данных)
  └── Collection (Коллекция) = Таблица в SQL
      └── Document (Документ) = Строка в SQL
          └── Field (Поле) = Колонка в SQL
```

## 🛠️ Практика 1: Командная строка

### Подключение к MongoDB
```bash
docker-compose exec mongodb mongosh --username student --password password
```

### Основные команды
```javascript
// Показать все базы данных
show dbs

// Создать/выбрать базу данных
use mystore

// Показать текущую БД
db

// Показать коллекции
show collections
```

### Создание документов
```javascript
// Вставить один документ
db.products.insertOne({
  name: "Ноутбук",
  price: 50000,
  category: "Электроника",
  inStock: true
})

// Вставить несколько документов
db.products.insertMany([
  {name: "Мышь", price: 1500, category: "Электроника", inStock: true},
  {name: "Книга", price: 800, category: "Образование", inStock: false},
  {name: "Телефон", price: 30000, category: "Электроника", inStock: true}
])
```

### Поиск документов
```javascript
// Найти все документы
db.products.find()

// Найти с условием
db.products.find({category: "Электроника"})

// Найти один документ
db.products.findOne({name: "Ноутбук"})

// Поиск с операторами
db.products.find({price: {$gt: 5000}})  // больше 5000
db.products.find({price: {$lt: 2000}})  // меньше 2000
db.products.find({inStock: true})       // равно true
```

### Обновление документов
```javascript
// Обновить один документ
db.products.updateOne(
  {name: "Ноутбук"},
  {$set: {price: 45000}}
)

// Обновить несколько документов
db.products.updateMany(
  {category: "Электроника"},
  {$set: {warranty: "2 года"}}
)

// Увеличить значение
db.products.updateOne(
  {name: "Мышь"},
  {$inc: {price: 200}}
)
```

### Удаление документов
```javascript
// Удалить один документ
db.products.deleteOne({name: "Книга"})

// Удалить несколько документов
db.products.deleteMany({inStock: false})

// Удалить всю коллекцию
db.products.drop()
```

## 🐍 Практика 2: Python + PyMongo

### Создать файл mongo_practice.py
```python
from pymongo import MongoClient

# Подключение
client = MongoClient("mongodb://student:password@localhost:27017/")
db = client.mystore
collection = db.products

# Вставка данных
products = [
    {"name": "Laptop", "price": 50000, "category": "Electronics"},
    {"name": "Mouse", "price": 1500, "category": "Electronics"},
    {"name": "Book", "price": 800, "category": "Education"}
]

result = collection.insert_many(products)
print(f"Вставлено {len(result.inserted_ids)} документов")

# Поиск
print("\nВсе товары:")
for product in collection.find():
    print(f"- {product['name']}: {product['price']} руб.")

print("\nЭлектроника:")
for product in collection.find({"category": "Electronics"}):
    print(f"- {product['name']}: {product['price']} руб.")

# Обновление
collection.update_one(
    {"name": "Laptop"},
    {"$set": {"price": 45000}}
)

# Подсчет
count = collection.count_documents({"category": "Electronics"})
print(f"\nТоваров в категории Electronics: {count}")

# Удаление
collection.delete_one({"name": "Book"})
print("Книга удалена")
```

### Запуск Python скрипта
```bash
# Создать файл в папке проекта
python mongo_practice.py
```

## 🎯 Задания для самостоятельной работы

### Задание 1: Интернет-магазин
Создать коллекцию `orders` с заказами:
```javascript
db.orders.insertMany([
  {
    orderId: 1001,
    customer: "Иван Петров",
    items: [
      {product: "Laptop", quantity: 1, price: 50000},
      {product: "Mouse", quantity: 2, price: 1500}
    ],
    total: 53000,
    status: "delivered",
    date: new Date()
  },
  {
    orderId: 1002,
    customer: "Мария Сидорова",
    items: [
      {product: "Phone", quantity: 1, price: 30000}
    ],
    total: 30000,
    status: "pending",
    date: new Date()
  }
])
```

**Задачи:**
1. Найти все заказы клиента "Иван Петров"
2. Найти заказы со статусом "pending"
3. Найти заказы на сумму больше 40000
4. Обновить статус заказа 1002 на "shipped"
5. Добавить поле "deliveryDate" к доставленным заказам

### Задание 2: Агрегация данных
```javascript
// Группировка по статусу
db.orders.aggregate([
  {$group: {_id: "$status", count: {$sum: 1}, totalAmount: {$sum: "$total"}}}
])

// Средняя сумма заказа
db.orders.aggregate([
  {$group: {_id: null, avgTotal: {$avg: "$total"}}}
])
```

### Задание 3: Индексы
```javascript
// Создать индекс для быстрого поиска
db.orders.createIndex({customer: 1})
db.orders.createIndex({status: 1})

// Посмотреть индексы
db.orders.getIndexes()
```

## 🔍 Полезные операторы

### Операторы сравнения
- `$eq` - равно
- `$ne` - не равно  
- `$gt` - больше
- `$gte` - больше или равно
- `$lt` - меньше
- `$lte` - меньше или равно
- `$in` - в массиве значений
- `$nin` - не в массиве значений

### Логические операторы
- `$and` - логическое И
- `$or` - логическое ИЛИ
- `$not` - логическое НЕ

### Операторы обновления
- `$set` - установить значение
- `$inc` - увеличить на число
- `$push` - добавить в массив
- `$pull` - удалить из массива

## 📊 Сравнение с SQL

| SQL | MongoDB |
|-----|---------|
| `SELECT * FROM products` | `db.products.find()` |
| `SELECT * FROM products WHERE price > 1000` | `db.products.find({price: {$gt: 1000}})` |
| `UPDATE products SET price = 2000 WHERE name = 'Mouse'` | `db.products.updateOne({name: "Mouse"}, {$set: {price: 2000}})` |
| `DELETE FROM products WHERE category = 'Old'` | `db.products.deleteMany({category: "Old"})` |
| `INSERT INTO products VALUES (...)` | `db.products.insertOne({...})` |

## 🎓 Домашнее задание

1. Создать коллекцию `users` с пользователями
2. Добавить 5 пользователей с полями: name, email, age, city
3. Написать запросы для поиска пользователей по возрасту и городу
4. Создать Python скрипт для работы с этой коллекцией
5. Добавить валидацию данных при вставке

## 📚 Дополнительные ресурсы

- [MongoDB Manual](https://docs.mongodb.com/manual/)
- [PyMongo Documentation](https://pymongo.readthedocs.io/)
- [MongoDB University](https://university.mongodb.com/) - бесплатные курсы